...
## P3 — Workflow manifests as GRAPHS, not lists — `ready` (next)
- **Goal**: declarative edges (`when`/`next`) on the step contract, validated closed at manifest build (every edge resolves, one entry, no cycle, all reachable, compensation ordering preserved); conditions as DATA over prior results and assembled context, never code; a `step.skipped` event recording which branch was taken and the values the predicate read.
- **Why**: this is the largest remaining reusable gap. Every governed workflow is a straight line, so "if the amount is under the tenant's threshold, skip the approval step" can only be modelled as two near-identical registered workflows — which puts the branch condition somewhere the journal never records and the policy engine never sees.
- **Deps**: P2. **Acceptance**: a sequential manifest behaves EXACTLY as it does today (the reference slice is the regression); a graph that cannot terminate is refused at `defineWorkflowManifest`, not at run time; fan-out either implemented or explicitly recorded as a limitation with its budget/idempotency/compensation interactions stated.
