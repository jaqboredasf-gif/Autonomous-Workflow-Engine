...
## D1 — The durable execution repository — `done (code + migration written and validated; 0017 NOT APPLIED)` (2026-07-28)
The successor three store headers had been naming since ADR-0002. Migration 0017: `awe_run_journals`, `awe_run_journal_entries` (append-only, hash-chained), `awe_run_leases`, `awe_run_results`; eleven `SECURITY DEFINER` functions with `search_path = ''` and `EXECUTE` revoked from `PUBLIC` before it is granted; RLS on with **zero** client policies (the G2 shape 0009 set and 0016 had to restore); append-only enforced by trigger so it holds against a superuser. **There is no `state` column** — a stored state could contradict the history it came from, so state stays a projection, and "claim the next runnable run" is named as a non-goal rather than half-built.
- **The database supplies atomicity and never rules.** Transitions, the chain, the projection and claim/renew/steal/fence stay pure in `awe-control-plane`. The SQL adds a compare-and-set two workers cannot both win, and asserts two safety invariants (a live lease cannot be taken from another holder; an appended entry must chain to the stored head).
- **The adapter is one method wide** (`call(fn, payload)`), so `@exattime/awe-runtime` still depends on no database driver and holds no credential. The same adapter runs over PostgREST in a deployment and over `psql` in a container in tests.
- **ADR-0002's Path C is demonstrated, not planned**: a role holding only `EXECUTE` on the eleven — no table privilege at all — passes the whole store contract and is refused every direct `SELECT`.
- **Files**: `supabase/migrations/0017_awe_durable_execution.sql`, `scripts/rollback-migration-0017.sql`, `packages/awe-runtime/src/postgres/{executor,journal-store,lease-store,result-store}.mjs`, `packages/awe-runtime/src/store-selection.mjs`, `packages/awe-runtime/src/{journal-store,result-store,lease-store,control-plane-service,index}.mjs`, `packages/awe-kernel/src/registry.mjs`, `packages/mcp-server/src/control-plane-tools.mjs`, `scripts/lib/{pg-harness.mjs,pg-shim.sql,store-conformance.mjs}`, `scripts/eval-durable-store.{mjs,sh}`, `scripts/{awe-control-plane,eval-control-plane}.mjs`, `docs/architecture/decisions/0010-durable-execution-repository.md`.
- **Acceptance**: Runner D **321/0** against a throwaway `postgres:17` (migrations 0001–0017 applied in order), **169/0** without Docker with the database gates reported as SKIP; Runner P **568/0** (was 564); `regression.sh --kinds=unit,offline,static` ALL GREEN, 3225 assertions. Two consecutive Runner D runs byte-identical. Rollback verified by round trip: applied → rolled back → replayed → re-applied, 24 pre-existing tables untouched.
- **Four defects found, all real**: (1) the result stores tenant-checked the READ and not the WRITE, so a second tenant silently replaced the first's step outputs; (2) the journal stores would accept a REWRITTEN prefix from a writer at the current head, and the chain still verified because a tampered history is internally consistent; (3) `awe_journal_write`'s create path was not serialized — `SELECT … FOR UPDATE` locks nothing when the row does not exist, so eight parallel creators all passed the compare-and-set and seven got a unique-violation error after doing the work; (4) an unawaited `claim()` in `cancelRun`, exposed within one run of the port change.
- **Port change, with evidence** (ADR-0010 D6): the store port is now `T | Promise<T>` and seven service reads are async. `await` on a plain value is a no-op, so the memory and file stores did not change; eleven call sites gained `await`.
- **Handoff**: the largest untested surface is `createSupabaseRpcExecutor` — the PostgREST binding — which no test covers because testing it needs a live project. Applying 0017 is human-gated and was not done.

## G1 — The Governed Agent Execution Plane — `done` (2026-07-28)
The layer that lets a specialized agent do real business work while staying tenant-bound, capability-limited, policy-constrained, approval-aware, durable, replayable and provider-neutral. The decision it implements is **ADR-0011: an agent proposes an action; the governed runtime authorizes and executes it.** Everything before this executed a step list declared in advance; an agent's next action is chosen at runtime, increasingly by a model, so the authorization decision had to move to the moment of action and out of the model's reach.
- **Five independent narrowings, in one function.** An action happens only if the agent definition declares (and does not deny) the capability, the capability binds that tool for that operation within its ceilings, the tenant grant exists and the control plane's policy engine allows it, the approval (where obliged) is in force and bound to these exact arguments, and the budget has room. Identity is checked before all five and before anything is read. The tenant-grant layer is `policy.mjs` **reused verbatim** — there is no second authorization implementation to drift.
- **A capability is a business permission, not a tool handle.** `record_invoice_draft` is a mechanism; "may prepare an AP draft, at or below internal sensitivity, with an idempotency key, medium risk" is a permission. Collapsing them is how an agent that "has the draft tool" may, by accident, draft anything for anyone.
- **An agent definition compiles to a workflow manifest** — one step per permitted (capability, operation, tool) binding — which is how `dispatch.mjs`, `policy.mjs` and the manifest rules are reused rather than reimplemented, and which makes an agent's complete action space one readable, digest-pinned document. A declared tool that no declared capability binds refuses compilation.
- **The model cannot authorize anything.** A planner gets a closed, redacted Planning View (no grant, no policy engine, no approval state, no credential, no environment, no executable handle) and returns a proposal parsed as untrusted input. Its claimed risk / side effect / approval requirement are recorded as belief; a planner that understates an approval requirement earns `planner_understated_approval` on a decision that requires approval anyway. Arguments beginning with `_` are refused by the grammar, because they are the runtime's reserved envelope.
- **An approval binds to the action and expires**: tenant, agent version, RESOLVED capability and tool versions, operation and arguments — commentary deliberately excluded, so rewording does not invalidate a decision and changing an argument does. A resume RE-AUTHORIZES from scratch and fails closed on definition or surface drift.
- **Seven `agent.*` events added to the existing journal**, all `to: null`, and the agent phase as a SECOND projection over the same entries. No second journal, no stored phase. `policy_denied` and `budget_exhausted` are the operator-facing distinction the control plane's single `failed` cannot make.
- **Four budgets** (turns, steps, tool calls, wall clock), each with its own reason, projected from the run's own history, all NOT NULL and > 0 — an unbounded agent cannot be configured. A refusal is never retried; the surface pins `max_attempts: 1`.
- **Improvement without self-modification**: evaluation is measurement captured before every terminal event including refusals, with a score derived from its deterministic checks; a candidate needs evidence, a human review and a human promotion; promotion produces a DRAFT that still cannot execute until a separate human activation and a redeploy. Rollback is automatic because nothing is ever replaced.
- **Files**: `packages/awe-agent/**` (NEW: capability, agent-definition, agent-registry, surface, proposal, authorization, planner, budget, harness, evaluation, reasons, kernel seam, index), `packages/awe-runtime/src/agent-service.mjs` (NEW), `packages/awe-runtime/src/reference/invoice-operations-agent.mjs` (NEW), `packages/awe-control-plane/src/{journal,index}.mjs` (additive), `packages/awe-runtime/src/index.mjs`, `packages/awe-kernel/src/registry.mjs`, `scripts/eval-governed-agent.{mjs,sh}` (NEW), `scripts/{eval-control-plane,eval-kernel}.mjs`, `scripts/lib/awe-reasons.mjs`, `docs/architecture/GOVERNED_AGENT_EXECUTION_PLANE.md` (NEW), `docs/architecture/decisions/0011-agents-propose-runtime-authorizes.md` (NEW).
- **Acceptance**: Runner G **379/0** over 51 fixtures, byte-identical across runs; Runner P **570/0** (was 568); `regression.sh --kinds=unit,offline,static` ALL GREEN, 3611 assertions. All 35 adversarial cases from the brief are covered and NAMED in their assertion messages, so a failure says which one broke.
- **Prompt injection is contained, not detected**, and the claim is deliberate. The intake fixture carries a real payload instructing the reader to skip approval, grant itself a capability and remit elsewhere; it is not filtered anywhere; Runner G drives planners that obey it completely and asserts they are refused every time.
- **Five defects found while building it**: identity checked after context assembly (a tenant-less run refused with the wrong reason); the approval binding read from a projection that knows nothing about proposals; a resume re-checking evidence against a bundle it no longer had; the resume time-budget segment starting at the approval rather than at the resume, so an operator's lunch break expired the run; and tool input carrying the whole observation history, which made an explicit idempotency key meaningless.
- **Handoff**: no model has ever driven this plane — the model boundary is a port returning a fixed string, which proves the contract and nothing about a real model's behaviour inside the view. No new schema was added; the plane persists through the ADR-0010 stores. Agent-run compensation is deliberately absent (the reference agent's pre-approval effects are internal drafts).

## G2 — A second governed agent, built only from data — `ready` (next)
- **Goal**: build a second specialized agent — dispatch, scheduling or crew coordination — by writing ONLY a definition, its capabilities, its tools and a deterministic planner. Write no new authorization code.
- **Why**: it is the only real test of whether G1 produced a reusable platform or an invoice-shaped one. Every claim in ADR-0011 about future agents inheriting the plane is unverified until a second agent exists. If the second agent needs new authorization code, THAT is the finding, and it belongs in the handoff rather than in a workaround.
- **Deps**: G1. **Acceptance**: the new agent passes an adversarial suite of its own reusing Runner G's shape; `packages/awe-agent` gains no new file and no new exported function; the two agents share the capability registry where their permissions genuinely overlap (`memory.read_operational`, `workflow.start`) and do not where they do not.
- **Watch for**: a capability that turns out to need a constraint the model does not express (a numeric ceiling, a time window, a per-actor limit). That is the most likely gap, and the answer is a capability field with its own rule — not a check inside a tool.

## D2 — Close the PostgREST gap — `ready`
- **Goal**: hold `createSupabaseRpcExecutor` to the same conformance contract the `psql` transport already passes, using a PostgREST-shaped executor double that reproduces supabase-js's `{ data, error }` envelope, its error codes, and its argument binding for a single `jsonb` parameter.
- **Why**: it is the one component in the durable path with no test at all, and it is the component a deployment actually runs. The `psql` transport proves the SQL and the adapter; it does not prove that binding.
- **Deps**: D1. **Acceptance**: the double's failure modes (a raised exception surfacing as `error`, a null `data`, a transport throw) each map to `StoreUnavailableError`; the shared conformance suite passes over the double; no live project is contacted.

## P4 — Fan-out and parallel steps — `ready`
...
- **What D1 changed about this**: the single-writer question now has a durable answer to design against. `awe_journal_write` serializes every append to a run through one row lock, so "appends are serialized through the single writer" is available as a *mechanism* and not only as a rule — the open question is whether a fan-out's branches should each take the lease in turn (simple, slow) or accumulate in one worker and commit once (fast, and the branch that fails mid-flight needs a defined story).
