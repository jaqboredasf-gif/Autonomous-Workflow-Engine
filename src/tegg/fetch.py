...
def session(
    ...
    *,
    ...
    ``knowledge`` is an open ``KnowledgeRun`` or None. When given, the sign-in
    uses what earlier runs worked out instead of rediscovering the form, and
    records whether that still held.
    """
    blockers = preflight()
    if blockers:
        raise FetchError(
    ...
    with sync_playwright() as playwright:
        ...
        try:
            explorer = Explorer(page, base_url, recorder, settings, knowledge=knowledge)
...
def list_completed(
    ...
    *,
    ...
    with session(
        base_url, recorder, settings, headless=headless, knowledge=knowledge
    ) as explorer:
