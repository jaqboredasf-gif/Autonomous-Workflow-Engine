...
**What that means in practice:** the back half of the process — everything that
happens once the documents are on disk — runs in under a second and is covered
by tests. The front half, pulling from the portal, is blocked on network access
to `tegg2.teggpro.com` rather than on code.
...
PYTHONPATH=src .venv/bin/python -m tegg.cli plan \
    --job config/job.example.yaml --drive-root /tmp/drive

# Split and merge into the final report
PYTHONPATH=src .venv/bin/python -m tegg.cli assemble \
    --job config/job.example.yaml --source /tmp/demo
```

The last command produces
`Acme Manufacturing Plant 3 - Toledo 2026 ESA Report.pdf` — 44 pages, correct
order — in about a third of a second.
...
config/job.example.yaml  the four inputs an operator supplies
...
src/tegg/cli.py          plan / assemble / fetch / run
```

An operator supplies only four values per job — company, site, agreement, site
visit. Everything else is fixed.
...
1. **Now** — use `assemble` on manually downloaded files. Removes the Acrobat
   splitting and merging entirely; no portal access required.
